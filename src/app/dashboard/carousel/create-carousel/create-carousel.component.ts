...
export class CreateCarouselComponent implements OnInit {
  ...
  get lauguageFormArray() {
    return this.createOlcarouselForm.controls.languages as FormArray;
  }

  get carouselFormArray() {
    return this.createOlcarouselForm.controls.carousels as FormArray;
  }

  constructor(private formBuilder: FormBuilder) {
    this.createOlcarouselForm = this.formBuilder.group({
      languages: new FormArray([]),
      carousels: new FormArray([])
    });
  ...
  getLanguageList() {
    ...
    this.languageList.forEach((x: { name: string, id: number }) => {
      this.lauguageFormArray.push(new FormControl(x.name == 'English' ? true : false));
      if (x.name == 'English') {
        this.carouselFormArray.push(new FormControl({ language: x.id, languageName: x.name }));
      }
    });
  }
  ...
  languageChange() {
    this.lauguageFormArray.controls.forEach((x, index)=>{
      ...
      if(x.value && !formControl){
          this.carouselFormArray.push(new FormControl({ language: language.id,  languageName: language.name }));
      }
