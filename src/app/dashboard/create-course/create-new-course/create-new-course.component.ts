...
export class CreateNewCourseComponent implements OnInit {
  ...
  constructor(
    ...
    if (this.router.getCurrentNavigation()?.extras.state != undefined) {
      ...
      this.selectedLanguages = JSON.parse(
        this.routergetdata['available_language']
      );
      ...
      this.showobjective = this.courceService.getTText(
        this.routergetdata.objective
      );
      this.showdescription = this.courceService.getTText(
        this.routergetdata.description
      );
      this.showforwhoom = this.courceService.getTText(
        this.routergetdata.for_whoom
      );
      this.showlearnmore = this.courceService.getTText(
        this.routergetdata.learn_more
      );
      if (this.learningType != '6') {
        if (this.routergetdata.resource != null) {
          let objectdata: any = {
            ...
            url:
              'https://orange.mindscroll.info/' + this.routergetdata.resource,
          };
      ...
      if (this.learningType == '3') {
        if (this.routergetdata.material_source != null) {
          let objectdata: any = {
            ...
            url:
              'https://orange.mindscroll.info/' +
              this.routergetdata.material_source,
          };
      ...
      if (this.routergetdata.learning_type == '1') {
        ...
        this.showILTLearnMore = this.courceService.getTText(
          this.routergetdata.learn_more
        );
        this.showILTForWhom = this.courceService.getTText(
          this.routergetdata.for_whoom
        );
        ...
        this.showCurriculumEmail =
          this.routergetdata.email_preffered_instructor;
        ...
        this.showCurriculumLearnMore = this.courceService.getTText(
          this.routergetdata.learn_more
        );
        ...
        this.showCurriculumRegional = Number(
          this.routergetdata.regional_cordinator
        );
        if (this.routergetdata.certification == 'yes') {
        ...
        this.showPlaylistTargetAudience =
          this.routergetdata['level'] == '1' ? 'yes' : 'no';
        this.showPlaylistEmail = this.routergetdata.for_whoom
          .toString()
          .replace('"', '')
          .replace('"', '');
      }
  ...
  ngOnInit(): void {
    ...
    if (this.routergetdata != undefined) {
      ...
      if (this.learningType == '1') {
        ...
        if (this.routergetdata.url != '') {
          this.materialbasedForm.get('url')?.setValidators(Validators.required);
    ...
    this.courceService.getLanguages().subscribe(
      (res: any) => {
        ...
        for (let index in languages) {
          ...
          const descriptionFormArr = formObj.controls
          ...
          const forWhomFormArr = this.iltandViltForm.controls
            .forWhomArr as FormArray;
          const learnMoreFormArr = this.iltandViltForm.controls
          ...
          const forWhomFormArr_cur = this.currriculumForm.controls
            .forWhomArr as FormArray;
          const learnMoreFormArr_cur = this.currriculumForm.controls
            .learnMoreArr as FormArray;
  ...
  get titlecontrol_playlist() {
    return (<FormArray>this.commonCreateCourceForm_playlist.get('titleArr'))
      .controls;
  }
  get titlecontrol_description() {
    return (<FormArray>this.commonCreateCourceForm.get('descriptionArr'))
      .controls;
  }
  ...
  addLearnerGuidelinetocurriculum(titleval: string, descriptionval: string) {
    return this.curriculumArray.push(
      this.addMorelearnerGuideline(titleval, descriptionval)
    );
  }
  ...
  createNewCource_Save(
    commonformObj: FormGroup,
    formObj: FormGroup,
    status: any
  ) {
    ...
    let courseid = {
      course_id: this.routergetdata == undefined ? null : this.routergetdata.id,
      resource_ext: '',
      material_source_ext: '',
    };
    ...
    if (!titlearray.includes('english')) {
      titlearray.push({
        english: commonformObj.value.title_single,
      });
    }
    ...
    if (this.learningType != '6') {
      ...
      if (!descriptionarray.includes('english')) {
        descriptionarray.push({
          english: commonformObj.value.description_single,
        });
      }
      if (!objectivearray.includes('english')) {
        objectivearray.push({
          english: commonformObj.value.objective_single,
        });
      }
    ...
    if (this.learningType == '1' || this.learningType == '4') {
      ...
      if (!for_whomarray.includes('english')) {
        for_whomarray.push({
          english: formObj.value.for_whoom_single,
        });
      }
      if (!learn_morearray.includes('english')) {
        learn_morearray.push({
          english: formObj.value.learn_more_single,
        });
      }
        ...
        if (this.routergetdata == undefined) {
          this.courceService.createCource(totalObj).subscribe(
            (res: any) => {
              console.log(res);
              if (res) {
                this.courceService.courseDetail(res.data.id).subscribe(
                  (res: any) => {
                    console.log(res);
                    ...
                    this.router.navigateByUrl(
                      '/dashboard/cources/request-detail',
                      {
          ...
          this.courceService.updateCourse(totalObj).subscribe(
            (res: any) => {
              ...
              if (res) {
                this.courceService
                  .courseDetail(this.routergetdata.id)
                  .subscribe(
                    (res: any) => {
                      console.log(res);
                      let coursedetail = res.data;
        ...
        this.courceService.updateCourse(totalObj).subscribe(
          (res: any) => {
            ...
            if (res) {
              ...
              let transferobj = {
                course_id: this.routergetdata.id,
          ...
          (res: any) => {
            ...
            if (res) {
              let transferobj = {
                ...
                transfer_id: this.selectedPublisherId,
                status: 'publish',
                intranet_url: this.publishForm.value.intranet_url,
                internet_url: this.publishForm.value.internet_url,
              };
  ...
  reject() {
    let statusobj = {
      course_id: this.routergetdata.id,
      status: 'reject',
      status_comment: this.rejectcomment,
    };
    this.courceService.changeStatus(statusobj).subscribe(
      (res: any) => {
        console.log(res);
        this.router.navigate(['/dashboard/cources']);
      },
  ...
  targetAudience(event: any) {
    if (event.id == 'yes') {
      ...
      this.playlistForm
        .get('free_field_content')
        ?.setValidators(Validators.required);
      this.playlistForm
        .get('email_preffered_instructor')
        ?.setValidators(Validators.required);
  ...
  formatdurationText($durationEvent: any) {
    ...
    if (!$duration.includes(':')) {
      if ($duration.length == 2) {
        ...
        $duration =
          '0' + $duration.substring(0, 1) + ':' + $duration.substring(1, 3);
  ...
  formatArrayData(
    ...
    for (let i = 0; i < fdata[fieldArr].length; i++) {
      if (fdata[fieldArr][i].value != '') {
        langArr.push({
          [`${fdata[fieldArr][i].name}`]: fdata[fieldArr][i].value,
        });
      }
    }
  ...
  FileConvertintoBytearray(file: any, cb: (arg0: any) => void) {
    // making File to Array Bytes
